...
USBFilesystem msFS6(myusb);
USBFilesystem msFS7(myusb);
USBFilesystem msFS8(myusb);
//USBFilesystem msFS9(myusb);
//USBFilesystem msFS10(myusb);
//USBFilesystem msFS11(myusb);
//USBFilesystem msFS12(myusb);
//USBFilesystem msFS13(myusb);
//USBFilesystem msFS14(myusb);
//USBFilesystem msFS15(myusb);
//USBFilesystem msFS16(myusb);
...
bool diskIO::mountFATPart(uint8_t partNum) {
	...
	if(drvIdx[idx].valid == true) return true;
...
void diskIO::connectedMSCDrives(void) {
  ...
  // Init all available USB drives for possible ext4 partitions.
 ...
 for (uint8_t i = 0; i < CNT_MSC; i++) {
	int drvSlot = drvIdx[i].driveID;
	// Setup EXT4 Partition.
	if((ml[i].pt == EXT4_TYPE) && !ml[i].mounted && bdl[i/4].connected && (ext4_mount_cnt < CONFIG_EXT4_BLOCKDEVS_COUNT)) {
		mountExt4Part(i);
	} else  if (*filesystem_list[i] && (filesystem_list[i]->partitionType != EXT4_TYPE)) {
		// Setup FAT32/EXFAT Partition.
		mountFATPart(i);
    } else if ((ml[i].pt == EXT4_TYPE) && (!bdl[i/4].connected) && (drvIdx[i].valid == true)) {
		// If not cleanly unmounted, complain, unmount and unregister EXT4 partition.
		if((ext4_mount_cnt > 0) && (ml[i].mounted == true)) {
			Serial.printf("\n**************** A BAD THING JUST HAPPENED!!! ****************\n");
			Serial.printf("When unmounting an EXT4 drive data is written back to the drive.\n");
			Serial.printf("To avoid losing cached data, unmount drive before removing device.\n");
			Serial.printf("Unmounting and removing drive (%d) partition (%d) from drive list. \n",i/4, i);
			Serial.printf("*****************************************************************\n\n");
    ...
    } else if((!*filesystem_list[i]) && (!bdl[i/4].connected) && (drvIdx[i].valid == true)) {
		//FAT/EXFAT Device disconnected reset device descriptor.
		Serial.printf("\n**************** A BAD THING JUST HAPPENED!!! ****************\n");
		Serial.printf("FAT32/EXFAT partition not cleanly unmounted.\n");
		Serial.printf("To avoid losing cached data, unmount drive before removing device\n");
		Serial.printf("Unmounting and removing drive (%d) partition (%d) from drive list. \n",i/4, i);
		Serial.printf("*****************************************************************\n\n");
...
/*
  ...
  drvIdx[slot].valid = true;
*/
  uint8_t slot = LOGICAL_DRIVE_SDSPI; // * SLOT number;

  // Init SD card (Block device 3) fixed.
  if(ext4fsp[SDIO_BD]->init_block_device(extsd, SDIO_BD) != EOK) {  
//Serial.printf("Init SDIO\n");
	// Device disconnected reset device descriptor.
    drvIdx[slot].fstype = nullptr;
    drvIdx[slot].valid = false;
	drvIdx[slot].ldNumber = 0;
	...
	if(ml[SDIO_MP].mounted) ext4fsp[SDIO_BD]->lwext_umount(SDIO_MP);
	// Else find and setup next avaiable device.
	if(currDrv == slot) findNextDrive();
    return false;
  }
  // if already connected and initialized return true (Nothing to do).
//  if(bdl[SDIO_BD].connected) return true;
  if(ml[SDIO_MP].pt == EXT4_TYPE) {
    if(!ext4fsp[SDIO_BD]->begin(SDIO_MP)) {
	  return false;
    }
    drvIdx[slot].ldNumber = slot;
